<?php
    ...
    if(isset($_SESSION['login'])){
        ...
        if(!has_Droit($idUser, "Consulter participation")){
            ...
            $fideles = $db->prepare("SELECT * FROM fidele, personne where fidele.personne_idpersonne = personne.idpersonne and fidele.lisible=1 and personne.lisible = 1 ORDER BY statut ASC LIMIT $premierElementDeLaPage, $nbeParPage");
            $fideles->execute();

            $sommeContribution = $db->prepare("SELECT `typecontribution`, fidele.idfidele,
                sum(case when fidele.idfidele IN(select fidele_idfidele from  contributionfidele) then (select sum(montant) from  contributionfidele, saintescene where fidele_idfidele = idfidele AND idsaintescene = saintescene_idsaintescene AND typecontribution = 'offrandes') else 0 end) as offrandes,
                sum(case when fidele.idfidele IN(select fidele_idfidele from  contributionfidele) then (select sum(montant) from  contributionfidele, saintescene where fidele_idfidele = idfidele AND idsaintescene = saintescene_idsaintescene AND typecontribution = 'don') else 0 end) as don,
                sum(case when fidele.idfidele IN(select fidele_idfidele from  contributionfidele) then (select sum(montant) from  contributionfidele, saintescene where fidele_idfidele = idfidele AND idsaintescene = saintescene_idsaintescene AND typecontribution = 'consiegerie') else 0 end) as consiegerie,
                sum(case when fidele.idfidele IN(select fidele_idfidele from  contributionfidele) then (select sum(montant) from  contributionfidele, saintescene where fidele_idfidele = idfidele AND idsaintescene = saintescene_idsaintescene AND typecontribution = 'Travaux') else 0 end) as travaux

                from contributionfidele, fidele where `saintescene_idsaintescene` = $idsaintescene AND fidele.lisible = true AND contributionfidele.lisible = 1 AND fidele_idfidele = idfidele group by  typecontribution");
